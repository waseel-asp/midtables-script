...
CREATE OR REPLACE VIEW NPHIES_COVERAGE_CLASS (COVERAGECLASSID , COVERAGEID, TYPE, VALUE, NAME) AS
	SELECT NPHIES_BENEFICIARY.rowid as COVERAGECLASSID , NPHIES_BENEFICIARY.rowid AS COVERAGEID, 'plan' AS TYPE, 'a' AS VALUE ,NULL AS NAME
	FROM WSL_GENINFO LEFT JOIN NPHIES_BENEFICIARY ON WSL_GENINFO.PROVCLAIMNO=NPHIES_BENEFICIARY.PROVCLAIMNO;
...
CREATE OR REPLACE VIEW NPHIES_CLAIMINFO (PROVCLAIMNO PRIMARY KEY DISABLE NOVALIDATE, EPISODEID, ISNEWBORN, CLAIMTYPE,
		...
		PREAUTHOFFLINEDATE, PREAUTHRESPONSEID, PREAUTHIDENTIFIERURL, PAYEETYPE, PAYEEID, COVERAGEID, BENEFICIARYID, SUBSCRIBERID, TOTAL,ISREFERRAL,REFERRINGPROVIDERNAME ,PRESCRIPTION, SPECIALITYREFERRALOFFLINEDATE, SPECIALITYREFERRALOFFLINEID, SPECIALITYREFERRALRESPONSEID) AS
	SELECT WSL_GENINFO.PROVCLAIMNO, WSL_GENINFO.PROVCLAIMNO AS EPISODEID, NULL, DEPTCODE AS CLAIMTYPE,
		...
		NPHIES_BENEFICIARY.BENEFICIARYID AS BENEFICIARYID,NULL, TOTCLAIMNETAMT AS TOTAL ,NULL AS ISREFERRAL,NULL AS REFERRINGPROVIDERNAME,NULL AS PRESCRIPTION, WSL_GENINFO.SPECIALITYREFERRALOFFLINEDATE AS SPECIALITYREFERRALOFFLINEDATE, WSL_GENINFO.SPECIALITYREFERRALOFFLINEID AS SPECIALITYREFERRALOFFLINEID, WSL_GENINFO.SPECIALITYREFERRALRESPONSEID AS SPECIALITYREFERRALRESPONSEID
	FROM WSL_GENINFO LEFT JOIN NPHIES_BENEFICIARY ON WSL_GENINFO.PROVCLAIMNO=NPHIES_BENEFICIARY.PROVCLAIMNO 
...
CREATE OR REPLACE VIEW NPHIES_CLAIMSUPPORTINGINFO(PROVCLAIMNO, SEQUENCENO, CATEGORY, REASON,
	...
	FROM WSL_GENINFO WHERE TRIM(MAINSYMPTOM) IS NOT NULL

	UNION ALL
	SELECT WG.PROVCLAIMNO AS PROVCLAIMNO, func_seq_test('supporting_sequence') AS SEQUENCENO, 'info', NULL, TO_CHAR(RADIOREPORT) AS SUPPORTINGVALUE, NULL, NULL, NULL, NULL AS CODE,
	NULL AS TIMINGPERIODFROM, NULL,NULL FROM WSL_GENINFO WG WHERE WG.RADIOREPORT IS NOT NULL AND DBMS_LOB.GETLENGTH(TRIM(RADIOREPORT))>0
    ...
    NPS.SERVICEDATE AS TIMINGPERIODFROM, NULL,NULL
    	FROM WSL_GENINFO WG JOIN NPHIES_PROVCLAIMNO_SERVICEDATE NPS ON NPS.PROVCLAIMNO = WG.PROVCLAIMNO WHERE WG.WEIGH IS NOT NULL AND WG.WEIGH != 0
    ...
    SELECT PROVCLAIMNO AS PROVCLAIMNO, func_seq_test('supporting_sequence') AS SEQUENCENO, 'physical-examination', NULL, TO_CHAR(SIGNIFICANTSIGN), NULL, NULL, NULL, NULL, NULL, NULL,NULL
        	FROM WSL_GENINFO WHERE TRIM(SIGNIFICANTSIGN) IS NOT NULL
    ...
    SELECT WG.PROVCLAIMNO AS PROVCLAIMNO, func_seq_test('supporting_sequence') AS SEQUENCENO, 'pulse', NULL, TO_CHAR(PULSE) AS SUPPORTINGVALUE, NULL, NULL, NULL, '/min' AS CODE ,
     NPS.SERVICEDATE AS TIMINGPERIODFROM, NULL,NULL
            	FROM WSL_GENINFO WG JOIN NPHIES_PROVCLAIMNO_SERVICEDATE NPS ON NPS.PROVCLAIMNO = WG.PROVCLAIMNO WHERE WG.PULSE IS NOT NULL AND WG.PULSE != 0
	...
	NPS.SERVICEDATE AS TIMINGPERIODFROM, NULL,NULL
    	FROM WSL_GENINFO WG JOIN NPHIES_PROVCLAIMNO_SERVICEDATE NPS ON NPS.PROVCLAIMNO = WG.PROVCLAIMNO WHERE TRIM(WG.BLOODPRESSURE) IS NOT NULL
	...
	SELECT WG.PROVCLAIMNO AS PROVCLAIMNO, func_seq_test('supporting_sequence') AS SEQUENCENO, 'last-menstrual-period', NULL, NULL AS SUPPORTINGVALUE, NULL, NULL, NULL, NULL AS CODE,
	 NPS.SERVICEDATE AS TIMINGPERIODFROM, NULL,NULL
	FROM WSL_GENINFO WG join NPHIES_PROVCLAIMNO_SERVICEDATE NPS ON NPS.PROVCLAIMNO = WG.PROVCLAIMNO
       WHERE LASTMENSTRUATIONPERIOD IS NOT NULL;
